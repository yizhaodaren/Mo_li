...
@implementation JANewReplyModel
...
- (CGFloat)cellHeight
{
    if (_cellHeight == 0 || self.refreshModel) {
        ...
        // 1 判断是否有回复指向
        if (![self.replyUser.userId isEqualToString:self.commentUserId]) {
            hasArrow = YES;
        }else{
            hasArrow = NO;
        }
        ...
        if (hasArrow) {   // 有回复指向
            ...
            beReplyNameFrameSize = [self.replyUser.userName sizeWithAttributes:@{NSFontAttributeName : JA_REGULAR_FONT(JA_PersonalDetail_commentPersonalFont)}];
            ...
            if (hasNameTag || hasCircleTag) {   // 前一个有标签
                ...
                if (hasNameTag && hasCircleTag) {
                    ...
                    // 箭头
                    _arrowFrame = CGRectMake(CGRectGetMaxX(_circleTagFrame) + 5, _nameFrame.origin.y, 30, 20);
                }else if (hasNameTag){
                    _nameTagFrame = CGRectMake(CGRectGetMaxX(_nameFrame) + 3, _nameFrame.origin.y + 2, 26, 14);
                    ...
                    _circleTagFrame = CGRectMake(CGRectGetMaxX(_nameFrame) + 3, _nameFrame.origin.y, 26, 14);
                    // 箭头
                    _arrowFrame = CGRectMake(CGRectGetMaxX(_circleTagFrame) + 5, _nameFrame.origin.y, 30, 20);
                }else{
                    _nameTagFrame = CGRectZero;
