 ...
 */
static NSInteger const kDefaultSpacingBetweenDots = 2;
...
- (void)changeActivity:(BOOL)active atIndex:(NSInteger)index
{
    if (self.dotViewClass) {
        ...
        if (active) {
            dotView.y = -(self.selectedDotSize.width-self.dotSize.width)/2.0;
            dotView.size = self.selectedDotSize;
        } else {
            dotView.y = 0;
            dotView.size = self.dotSize;
        }
...
- (CGSize)selectedDotSize {
    if (self.dotViewClass && CGSizeEqualToSize(_selectedDotSize, CGSizeZero)) {
