    ...
    switch (status) {
        ...
        case NIMInputStatusAudio:{
            if (self.toolBar.showsKeyboard) {
                self.nim_top = self.keyBoardFrameTop - self.toolBar.nim_height - offY;
            }else{
                self.nim_top = self.superview.nim_height - self.toolBar.nim_height - offY;
            }
            break;
        }
        case NIMInputStatusMore:
        case NIMInputStatusEmoticon:
            self.nim_bottom = self.superview.nim_height - offY;
            break;
        default:
            break;
    }
...
- (NIMInputMoreContainerView *)moreContainer
{
    if (!_moreContainer) {
        ...
        _moreContainer.backgroundColor = HEX_COLOR(JA_BtnGrounddColor);
...
#warning 修改-64
    self.keyBoardFrameTop = endFrame.origin.y-64;
...
- (void)layoutSubviews
{
    ...
    if (iPhoneX) {
        CGPoint point = [self convertPoint:CGPointMake(0, self.toolBar.nim_bottom) toView:self.superview];
        BOOL showInputView = point.y != self.superview.nim_height - 34;
        if (!showInputView) {
            margin = 34;
        }
    }
    
    _moreContainer.nim_top     = self.toolBar.nim_bottom+margin;
    _emoticonContainer.nim_top = self.toolBar.nim_bottom+margin;
}
...
//    NSString *str = [self.toolBar.contentText stringByAppendingString:text];
    // text 可能未nil导致crash
    NSString *str = self.toolBar.contentText;
    if (text.length) {
