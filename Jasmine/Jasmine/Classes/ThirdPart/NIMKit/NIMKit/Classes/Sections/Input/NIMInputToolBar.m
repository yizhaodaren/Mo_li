...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        self.backgroundColor = HEX_COLOR(JA_BtnGrounddColor);
        ...
        _recordButton.layer.borderColor = [HEX_COLOR(0xBDBDBD) CGColor];
        _recordButton.layer.borderWidth = JA_SCREEN_ONE_PIEXL;
...
//        [_inputTextBkgImage setImage:[[UIImage nim_imageInKit:@"icon_input_text_bg"] resizableImageWithCapInsets:UIEdgeInsetsMake(15,80,15,80) resizingMode:UIImageResizingModeStretch]];
        _inputTextBkgImage.backgroundColor = [UIColor whiteColor];

        _inputTextBkgImage.layer.cornerRadius = 4;
        _inputTextBkgImage.layer.masksToBounds = YES;
        _inputTextBkgImage.layer.borderColor = [HEX_COLOR(0xBDBDBD) CGColor];
        _inputTextBkgImage.layer.borderWidth = JA_SCREEN_ONE_PIEXL;
...
//        _inputTextView.backgroundColor = [UIColor redColor];
        ...
        _topsep.backgroundColor = HEX_COLOR_ALPHA(0x000000, 0.1);
        [self addSubview:_topsep];
        
        _bottomsep = [[UIView alloc] initWithFrame:CGRectZero];
        _bottomsep.backgroundColor = HEX_COLOR_ALPHA(0x000000, 0.1);
...
- (void)layoutSubviews{
    ...
    //底部分割线
    CGFloat sepHeight = JA_SCREEN_ONE_PIEXL;
    _topsep.nim_size     = CGSizeMake(self.nim_width, sepHeight);
...
- (void)adjustTextAndRecordView
{
    if ([self.types containsObject:@(NIMInputBarItemTypeTextAndRecord)])
    {
        ...
        if (!self.recordButton.superview)
        {
            ...
            self.recordButton.y = (self.frame.size.height-self.inputTextBkgImage.height)/2.0;
