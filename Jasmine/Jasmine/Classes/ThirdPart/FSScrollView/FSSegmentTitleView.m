...
@interface FSSegmentButton : UIButton

@property (nonatomic, strong) UIView *redView;

@end

@implementation FSSegmentButton
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        redView.layer.borderColor = HEX_COLOR(0xffffff).CGColor;
...
- (void)layoutSubviews {
    ...
    self.redView.x = self.width - self.redView.width - 3;
    self.redView.y = 7;
}
...
@property (nonatomic, strong) NSMutableArray<FSSegmentButton *> *itemBtnArr;
...
- (void)initWithProperty
{
    self.itemMargin = 20;
    self.selectIndex = 0;
    self.titleNormalColor = HEX_COLOR(JA_ListTitle);
    self.titleSelectColor = HEX_COLOR(JA_Green);
    self.titleFont = JA_REGULAR_FONT(15);
...
- (void)layoutSubviews
{
    ...
    if (_titleFont != _titleSelectFont) {
        for (int idx = 0; idx < self.titlesArr.count; idx++) {
            FSSegmentButton *btn = self.itemBtnArr[idx];
    ...
    if (totalBtnWidth <= CGRectGetWidth(self.bounds)) {//不能滑动
        ...
        [self.itemBtnArr enumerateObjectsUsingBlock:^(FSSegmentButton * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
{
    UIFont *titleFont;
    FSSegmentButton *selectBtn = self.itemBtnArr[self.selectIndex];
    ...
    [UIView animateWithDuration:(animated?0.25:0) animations:^{
        switch (_indicatorType) {
            ...
            case FSIndicatorTypeCustom:
                self.indicatorView.center = CGPointMake(selectBtn.center.x, CGRectGetHeight(self.scrollView.bounds) - 1);
                self.indicatorView.bounds = CGRectMake(0, 0, 12, 2);
                break;
        ...
        self.oriX = self.indicatorView.x;
...
- (NSMutableArray<FSSegmentButton *>*)itemBtnArr
{
    if (!_itemBtnArr) {
...
- (void)setTitlesArr:(NSArray *)titlesArr
{
    ...
    for (NSString *title in titlesArr) {
        FSSegmentButton *btn = [FSSegmentButton buttonWithType:UIButtonTypeCustom];
...
- (void)setSelectIndex:(NSInteger)selectIndex
{
    ...
    FSSegmentButton *lastBtn = [self.scrollView viewWithTag:_selectIndex + 666];
    lastBtn.selected = NO;
    lastBtn.titleLabel.font = _titleFont;
    _selectIndex = selectIndex;
    FSSegmentButton *currentBtn = [self.scrollView viewWithTag:_selectIndex + 666];
...
- (void)setStartIndex:(NSInteger)startIndex
{
    ...
    CGFloat startindicatorWidth = [FSSegmentTitleView getWidthWithString:self.titlesArr[self.startIndex] font:_titleFont];
    self.oriWidth = startindicatorWidth;
...
// 
- (void)setCurrentOffX:(CGFloat)currentOffX
{
    _currentOffX = currentOffX;
    
    // 开始索引  -- 获取按钮
    FSSegmentButton *startBtn = self.itemBtnArr[self.startIndex];
    CGFloat startindicatorWidth = [FSSegmentTitleView getWidthWithString:self.titlesArr[self.startIndex] font:_titleFont];

    // 结束索引
    FSSegmentButton *endBtn = self.itemBtnArr[self.endIndex];
    CGFloat endindicatorWidth = [FSSegmentTitleView getWidthWithString:self.titlesArr[self.endIndex] font:_titleFont];
    
    // 计算需要移动的距离
    CGFloat margin1 = (endBtn.width - endindicatorWidth) * 0.5 + endindicatorWidth;
    CGFloat margin2 = (startBtn.width - startindicatorWidth) * 0.5;
    CGFloat off = margin1 + margin2;
    
    self.indicatorView.width = self.oriWidth + off * currentOffX * 0.7;
    
    if (self.endIndex < self.startIndex) {  // 右
     
        self.indicatorView.x = self.oriX - off * currentOffX * 0.7;
    }
}
...
- (void)setTitleFont:(UIFont *)titleFont
{
    ...
    for (FSSegmentButton *btn in self.itemBtnArr) {
...
- (void)setRedPointViewHidden:(BOOL)hidden index:(NSInteger)index {
    if (index < self.itemBtnArr.count) {
        FSSegmentButton *btn = self.itemBtnArr[index];
        btn.redView.hidden = hidden;
    }
}

- (void)setShowRedViewIndex:(NSInteger)showRedViewIndex {
  
}
#pragma mark --Btn

- (void)btnClick:(FSSegmentButton *)btn
{
