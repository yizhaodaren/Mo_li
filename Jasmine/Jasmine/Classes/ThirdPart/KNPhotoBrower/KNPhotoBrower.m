...
//       白色删除   KNPhotoBrower.bundle/more_tap@2x.png
    [operationBtn setImage:[UIImage imageNamed:@"more_grzy"] forState:UIControlStateNormal];
    [operationBtn setFrame:(CGRect){{ScreenWidth - 35 - 15,25},{56,27}}];
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex == 1) {
        // 1: 删除后 回调返回 绝对 下标
        if([self.delegate respondsToSelector:@selector(photoBrowerRightOperationDeleteImageSuccessWithAbsoluteIndex:)]){
            [self.delegate photoBrowerRightOperationDeleteImageSuccessWithAbsoluteIndex:_currentIndex];
        }
        [self deleteImageIBAction];
    }
}
...
- (void)operationBtnIBAction{
    ...
    // 自己满足需求 现加代码 ===============
//    UIAlertView *alertV = [[UIAlertView alloc] initWithTitle:@"确定要删除图片" message:nil delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
...
//  如果传入的 ActionSheetArr 有下载图片这一选项. 则在这里调用和下面一样的方法 switch.....,如果没有下载图片,则通过代理方法去实现...
            ...
            // 自己的代码 ====== 自己的代码
            NSString *title = nil;
            if (buttonIndex < weakSelf.actionSheetArr.count) {
                title = weakSelf.actionSheetArr[buttonIndex];
