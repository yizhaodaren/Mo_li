...
@property (nonatomic, strong) JAPaddingLabel *redCountLabel;
...
- (void)initialize {
    UIView *redPointView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 6, 6)];
    _redPointView = redPointView;
    redPointView.backgroundColor = HEX_COLOR(0xFE3824);
...
//    redPointView.layer.borderColor = HEX_COLOR(0xffffff).CGColor;
//    redPointView.layer.borderWidth = 1;
    [self addSubview:redPointView];
    
    JAPaddingLabel *redCountLabel = [JAPaddingLabel new];
    _redCountLabel = redCountLabel;
    redCountLabel.backgroundColor = HEX_COLOR(0xFF3B30);
    redCountLabel.userInteractionEnabled = NO;
    redCountLabel.hidden = YES;
    redCountLabel.layer.cornerRadius = 8;
    redCountLabel.layer.masksToBounds = YES;
    redCountLabel.font = JA_REGULAR_FONT(10);
    redCountLabel.textColor = HEX_COLOR(0xffffff);
    redCountLabel.textAlignment = NSTextAlignmentCenter;
    redCountLabel.edgeInsets = UIEdgeInsetsMake(1, 5, 1, 5);
    [self addSubview:redCountLabel];
...
- (void)layoutSubviews {
    ...
    self.redPointView.x = self.width-2;
    self.redPointView.y = 10;
    
    self.redCountLabel.x = self.width-2;
    self.redCountLabel.y = 15-8;
    self.redCountLabel.height = 16;
}
...
- (void)initialize {
    ...
    dividingLine.backgroundColor = HEX_COLOR(0xf4f4f4);
...
- (void)buttonInPageMenuClicked:(SPItem *)sender {
    ...
    if (self.trackerStyle == SPPageMenuTrackerStyleTextZoom) {
        ...
        if (fromIndex != toIndex) { // 如果相等，说明是第一次进来，或者2次点了同一个，此时不需要动画
            ...
            if (self.delegate && [self.delegate respondsToSelector:@selector(pageMenu:scrollToTopToIndex:)]) {
                [self.delegate pageMenu:self scrollToTopToIndex:toIndex];
            }
        }
    }
...
//        CGFloat originX = fromButton.frame.origin.x+(fromButton.frame.size.width-fromButton.titleLabel.font.pointSize)*0.5;
        CGFloat originX = fromButton.frame.origin.x+(fromButton.frame.size.width-trackerWidth)*0.5;
        // 原先的宽度
        CGFloat originW = trackerWidth;
        if (currentOffsetX - _beginOffsetX >= 0) { // 向左拖拽了
            if (progress < 0.5) {
...
//        if (currentOffsetX - _beginOffsetX >= 0) { // 向左拖拽了
//            if (progress < 0.5) {
//                originX = fromButton.frame.origin.x;
//                originW = fromButton.frame.size.width;
//                xDistance = (_itemPadding+toButton.frame.size.width);
//
//                newFrame.origin.x = originX; // x值保持不变
//                newFrame.size.width = originW + xDistance * progress * 2;
//            } else {
//                originX = fromButton.frame.origin.x;
//                originW = toButton.frame.size.width;
//                xDistance = (_itemPadding+fromButton.frame.size.width);
//
//                newFrame.origin.x = originX + xDistance * (progress-0.5) * 2;
//                newFrame.size.width = (originW + xDistance) - xDistance * (progress-0.5) * 2;
//            }
...
//                newFrame.size.width = originW + xDistance * progress * 2;
//            } else {
//                originX = toButton.frame.origin.x;
//                originW = toButton.frame.size.width;
//                xDistance = (_itemPadding+fromButton.frame.size.width);
//
...
- (void)setSelectedItemIndex:(NSUInteger)selectedItemIndex {
    ...
    if (self.buttons.count) {
        ...
        [self scrollInPageMenuClicked:button];
    }
}

// 滚动的方式修改button(和点击事件处理一样，为了区别开，新增此方法)
- (void)scrollInPageMenuClicked:(SPItem *)sender {
    [self.selectedButton setTitleColor:_unSelectedItemTitleColor forState:UIControlStateNormal];
    [sender setTitleColor:_selectedItemTitleColor forState:UIControlStateNormal];
...
- (void)setTitleContentInset:(UIEdgeInsets)titleContentInset {
    _titleContentInset = titleContentInset;
    for (SPItem *button in self.buttons) {
        button.titleEdgeInsets = titleContentInset;
    }
    [self setNeedsLayout];
    [self layoutIfNeeded];
    // 修正scrollView偏移
...
- (void)layoutSubviews {
    ...
    CGFloat dividingLineH = (self.dividingLine.hidden || self.dividingLine.alpha < 0.01) ? 0 : _trackerHeight;
    ...
    CGFloat itemScrollViewH = backgroundViewH;
    ...
    [self.buttons enumerateObjectsUsingBlock:^(SPItem *button, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        } else if (self.permutationWay == SPPageMenuPermutationWayNotScrollCustom) {
            buttonW = [buttonWidths[idx] floatValue];
            CGFloat padding = (JA_SCREEN_WIDTH-buttonW*self.buttons.count-_itemPadding*(self.buttons.count-1))/2.0;
            if (idx == 0) {
                button.frame = CGRectMake(padding+lastButtonMaxX, 0, buttonW, itemScrollViewH);
            } else {
                button.frame = CGRectMake(_itemPadding+lastButtonMaxX, 0, buttonW, itemScrollViewH);
...
- (void)setRedPointViewHidden:(BOOL)hidden index:(NSInteger)index {
    if (index < self.buttons.count) {
        SPItem *btn = self.buttons[index];
        btn.redPointView.hidden = hidden;
    }
}

- (void)setRedCountLabelValue:(NSString *)value index:(NSInteger)index {
    if (index < self.buttons.count) {
        SPItem *btn = self.buttons[index];
        btn.redCountLabel.hidden = value.length?NO:YES;
        btn.redCountLabel.text = value;
        [btn.redCountLabel sizeToFit];
    }
}
