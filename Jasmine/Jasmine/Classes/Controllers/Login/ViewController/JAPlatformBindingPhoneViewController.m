//
//  JAPlatformBindingPhoneViewController.m
//  Jasmine
//
...
@interface JAPlatformBindingPhoneViewController ()<UITextFieldDelegate>
...
@implementation JAPlatformBindingPhoneViewController
...
- (void)textFieldChanged:(NSNotification *)note{
    ...
    if (self.phoneNumT.text.length&&self.vertifyNumT.text.length) {
...
- (void)setUpUI{
    ...
    self.bindBtn = [[UIButton alloc] initWithFrame:CGRectMake(20, separate2.bottom+121, JA_SCREEN_WIDTH  - 40, 50)];
    ...
    self.bindBtn.enabled = NO;
    [self.bindBtn setTitle:@"绑定手机" forState:UIControlStateNormal];
...
- (void)getVertfy{
    ...
    if (![JARegex RegexMobileNumber:self.phoneString]) {
        ...
        senDic[JA_Property_CodeType] = @"三方注册绑定手机";
...
- (void)bindPhoneLogin:(UIButton *)btn
{
    ...
    // 神策数据
    NSMutableDictionary *senDic = [NSMutableDictionary dictionary];
    NSString *type = self.platformInfo[@"platformType"];
    ...
    NSMutableDictionary *dic = [NSMutableDictionary dictionary];
    [dic setValuesForKeysWithDictionary:self.platformInfo];
    ...
    [[JAUserApiRequest shareInstance] regisetUserAccount:dic success:^(NSDictionary *result) {
        ...
        // 登录成
        [JAAPPManager app_loginSuccessWithResult:result loginType:type.integerValue];
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            if (type.integerValue == 1) {
                [[JACommonApi shareInstance] addDataLogs_registerWithType:@"qq"];
            }else if (type.integerValue == 2){
                [[JACommonApi shareInstance] addDataLogs_registerWithType:@"wx"];
            }else if (type.integerValue == 3){
                [[JACommonApi shareInstance] addDataLogs_registerWithType:@"wb"];
            }
