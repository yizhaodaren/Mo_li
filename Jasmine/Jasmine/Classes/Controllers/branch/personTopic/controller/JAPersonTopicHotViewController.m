//
//  JAPersonTopicHotViewController.m
//  Jasmine
//
...
@interface JAPersonTopicHotViewController ()
...
@implementation JAPersonTopicHotViewController
...
- (void)viewDidLoad {
    ...
    self.tableView.mj_footer = [MJRefreshAutoNormalFooter footerWithRefreshingBlock:^{
        @StrongObj(self);
        [self request_getTopicVoiceHotListWithMore:YES];
    }];
    self.tableView.mj_footer.hidden = YES;
    
    [self request_getTopicVoiceHotListWithMore:NO];
}

#pragma mark - 网络请求
- (void)request_getTopicVoiceHotListWithMore:(BOOL)isMore
{
    if (!isMore) {
        self.currentPage = 1;
        self.tableView.mj_footer.hidden = YES;
