...
/**
    发射范围规则 ： 一共分9段，一段是13度，每段中间间隔1度 那么发射范围就是 9 * 13 + 8 = 125
 
    发射角度规则 ： 每个轨道从上面9段中任取1段（不重复），在该段的角度范围中任取一个角度作为发射角度
 
    动画规则 :  动画1 ：数字弹出动画 y轴上移（距离：自身高度+10）  同时缩放（比例： 0-1.2-0.9-1.0） 动画总时长0.3秒  消失动画 ：0.3秒淡出
               动画2 ： 每次可被10整除时，如果数字正在展示的时候，执行 缩放动画（比例 1-1.2-0.9-1.0） 时长0.2秒
 
               count = 20 鼓励变加油 同时执行动画1
               count = 40 加油变太棒了 同时执行动画1
               count = 其他被10整除的数字  执行动画2
 
    计数规则：单击一次count + 1，长按时 每0.1秒 count + 1 ， 点赞取消时 count 清0
 
    点赞和取消 ： 单击： 没有赞的状态 单击点赞（执行动画），下次点击和上次点击间隔在1.3秒内，算相同的点击（继续动画），超过1.3秒执行取消点赞（不动画）
                      有赞的状态  点击取消点赞（不动画） ，下次点击 - 同上
 
                长按： 没有赞的状态，长按执行点赞并执行动画， 有赞的状态，直接执行动画
 
    初始速度、x加速度、y加速度
            CGFloat v = (arc4random() % 300 + 200) * JA_SCREEN_SCALE / 0.8; -- 宽度是像素值
            CGFloat yA = - v * sin(发射角度) + 300;
            CGFloat xA = - (v * cos(发射角度));
 */
...
- (void)touchDown
{
    if (!IS_LOGIN) {
        ...
        dic[JA_Property_BindingType] = @"点赞";
...
//        cell.alphaSpeed = -1;
        ...
        // 初始速度
        CGFloat v = (arc4random() % 300 + 200) * JA_SCREEN_SCALE / 0.8;
