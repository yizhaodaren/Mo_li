...
@implementation JANewVoiceWaveView
...
//        self.sliderIV.drawColor = HEX_COLOR(0xffffff);

        self.sliderIV = [UIImageView new];
        self.sliderIV.image = [UIImage imageNamed:@"voice_wave_slider"];
        self.sliderIV.frame = CGRectMake(0, -1, 11, frame.size.height+4);
...
//    self.layer.contents = (id)[UIImage imageNamed:imageName].CGImage;
}
...
- (void)drawRect:(CGRect)rect{
    if ([_peakLevelQueue count] > 0) {
        ...
        CGFloat volumeBarWidth = 2.5;
        CGFloat volumeBarXOffset = 1.5;
        CGFloat volumeBarX = volumeBarWidth;// 起始位置
